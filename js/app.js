...
function loopLevelArr(arr, dest) {
    arr.forEach(function(row, rowIndex) {
        row.forEach(function(element, colIndex) {
            var location = new Vector(colIndex * 101, -20 +
                rowIndex * 83);
            ...
            switch (element) {
                ...
                case 'og':
                    ...
                    break;
                case 'hh':
                    dest.push(new Bug(location, MIN_BUG_SPEED,
                        "horizontal"));
                    break;
                case 'vv':
                    dest.push(new Bug(location, MIN_BUG_SPEED,
...
Entity.prototype.render = function() {
    if (this.sprite) {
        ctx.drawImage(Resources.get(this.sprite), this.pos.getX(), this.pos
            .getY());
    };
};
...
Bug.prototype.update = function(dt) {
    ...
    if (this.speed > 0) {
        this.speed = this.speed < BUG_INT_SPEED ? this.speed += dt * 100 :
            this.speed = this.speed < MAX_BUG_SPEED ? this.speed += dt * 9 :
            this.speed;
    } else {
        this.speed = this.speed > -BUG_INT_SPEED ? this.speed -= dt * 100 :
            this.speed = this.speed > -MAX_BUG_SPEED ? this.speed -= dt * 9 :
...
Bug.prototype.checkCollision = function(distanceVector) {
    ...
    levels[currentLevel].objects.forEach(function(thing) {
        if (thing.id !== self.id)
            if (thing.type === 'obstacle' || thing.type === 'enemy' ||
                thing.type === 'princess') {
                if (touch(self, thing)) {
                    ...
                    self.displayDirection = self.displayDirection ?
                        false : true;
...
Bug.prototype.render = function() {
    if (this.direction === "horizontal") {
        if (this.displayDirection) {
            ...
            ctx.drawImage(Resources.get(this.sprite), this.pos.getX(), this
                .pos.getY());
        } else {
 ...
 */
var Collectible = function(pos, sprite, type, lifeEffect, scoreIncrease,
    collected) {
...
Player.prototype.checkCollisions = function(currentLevel) {
    ...
    if (runningGame) {
        levels[currentLevel].objects = levels[currentLevel].objects.concat(
            collecteGems);
    };
};
